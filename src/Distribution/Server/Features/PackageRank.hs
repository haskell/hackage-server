...
rankPackageIO download upload p = maintNum
    where
            ...
            maintNum :: IO Double
            maintNum = do  
                        maint <- queryUserGroups $ 
                                [maintainersGroup upload pkgNm]
...
rankPackagePure p = reverseDeps+usageTrend+docScore+stabilityScore
    ...
    where   reverseDeps = 1
            ...
            testsBench = (bool2Double . hasTests) p 
                            + (bool2Double . hasBenchmarks) p
            goodMetadata = 1
            weightUniqueDeps = 1
...
rankPackage download upload p = rankPackageIO download upload p 
