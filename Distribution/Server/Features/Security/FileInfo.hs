...
instance FileInfo Snapshot where
  fileInfo Snapshot{..} = mkFileInfo snapshotLength snapshotHashSHA256

instance FileInfo Root where
  fileInfo Root{..} = mkFileInfo rootLength rootHashSHA256

instance FileInfo Mirrors where
  fileInfo Mirrors{..} = mkFileInfo mirrorsLength mirrorsHashSHA256
