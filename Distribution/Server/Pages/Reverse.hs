...
reversePackageRender :: PackageId -- ^ The package whose information is displayed.
                     ...
                     -> ReverseResource -- ^ The resource for generating revdeps-related URIs.
                     ...
                     -> [Html]
reversePackageRender pkgid packageLink r isRecent (ReversePageRender renders counts total) =
  ...
  where
    ...
    reverseTableRows =
        ...
        [ tr ! [theclass (if odd n then "odd" else "even")] <<
            ...
            , td << [ toHtml $ (show count) ++ " (", anchor ! [href $ reverseNameUri r "" $ packageName pkg] << "view", toHtml ")" ] ]
...
reverseFlatRender pkgname packageLink r (ReverseCount total flat _) pairs =
  ...
  where
    ...
    reverseTableRows =
        ...
        [ tr ! [theclass (if odd n then "odd" else "even")] <<
            [ td << toPackage pkg
            , td << [ toHtml $ (show count) ++ " (", anchor ! [href $ reverseAllUri r "" pkg] << "view", toHtml ")" ]
            ]
...
reverseStatsRender pkgname allVersions packageLink r (ReverseCount total flat versions) =
  ...
  [ case total of
        ...
        _ -> toHtml
            [ paragraph << [ anchor ! [href $ reverseNameUri r "" pkgname] << num total "packages" "package"
            ...
            , paragraph << [ anchor ! [href $ reverseAllUri r "" pkgname] << num flat "packages" "package"
    ...
    versionTableRows =
        ...
        [ tr ! [theclass (if odd n then "odd" else "even")] <<
            ...
            , td << [ toHtml $ show (Map.findWithDefault 0 version versions) ++ " ("
                    , anchor ! [href $ reverseUri r "" pkgid] << "view", toHtml ")" ]
            ]
...
reversePackageSummary pkgid r (direct, version) = (,) "Reverse dependencies" $
  if direct == 0
    then toHtml "None"
                ...
                , toHtml " and "
                , anchor ! [href $ reverseNameUri r "" (packageName pkgid) ] << show direct
                , toHtml " total"]
